...
mi_dfs <- readRDS("factorial/sensitivity analysis/search only/se02_mi_dfs.RDS") 

# Loss to follow-up weights
crosssec_df <- readRDS(paste0(path_diabetes_subphenotypes_youth_folder,"/working/cleaned/factorial/search only/se01_cross sectional df.RDS")) %>% 
  dplyr::select(-"nonna_selfmnsi") %>% 
  mutate(cs_available = case_when(is.na(combined_abnormal) ~ 0,
                                  TRUE ~ 1))

cs_available_mod <- glm(cs_available ~ cluster + age_category + female + race_eth, 
                     data = crosssec_df, 
...
bind_rows(
  ...
  write_csv(.,"factorial/sensitivity analysis/search only/se03_cross-sectional coefficients.csv")
