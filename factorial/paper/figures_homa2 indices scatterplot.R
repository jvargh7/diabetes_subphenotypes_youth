...
homa2_df <- read_excel("factorial/analysis/HOMA2Calculator.xlsm", sheet = "Insulin") %>% 
  slice(1:337) %>% 
  rename(homa2b = `HOMA2 %B`, homa2ir = `HOMA2 IR`) %>%  
  dplyr::select(-`HOMA2 %S`) %>% 
  mutate(obs = row_number()) 

prep_df <- read_excel("factorial/analysis/dsy05_homa2 indices calculation.xlsx") %>% 
  mutate(obs = row_number()) %>% 
...
combined_plot %>% 
  ggsave(plot = ., filename = paste0(path_diabetes_subphenotypes_youth_folder,"/figures/factorial/homa2 indices by ratio and cpeptide.jpg"),width = 14, height = 5.5)
