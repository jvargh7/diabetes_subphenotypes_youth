...
mi_dfs <- readRDS("etiologic/sensitivity analysis/search only/se02_mi_dfs.RDS") 

# Loss to follow-up weights
crosssec_df <- readRDS(paste0(path_diabetes_subphenotypes_youth_folder,"/working/cleaned/etiologic/search only/se01_cross sectional df.RDS")) %>% 
...
bind_rows(
  ...
  write_csv(.,"etiologic/sensitivity analysis/search only/se03_cross-sectional coefficients.csv")
