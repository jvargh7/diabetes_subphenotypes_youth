...
mi_dfs <- readRDS("prov/sensitivity analysis/search only/se02_mi_dfs.RDS") 

# Loss to follow-up weights
crosssec_df <- readRDS(paste0(path_diabetes_subphenotypes_youth_folder,"/working/cleaned/prov/search only/se01_cross sectional df.RDS")) %>% 
...
bind_rows(
  ...
  write_csv(.,"prov/sensitivity analysis/search only/se03_cross-sectional coefficients.csv")
