...
search <- readRDS(paste0(path_diabetes_subphenotypes_youth_folder,"/working/search/search_prov.RDS")) %>% 
...
#write.csv(source_df, paste0(path_diabetes_subphenotypes_youth_folder,"/working/cleaned/prov/setdy01c_source sample.csv"))

analytic_df = source_df %>% 
    dplyr::filter(!is.na(hba1c) | !is.na(cpeptidef) | !is.na(totalc) | !is.na(ldlc) | !is.na(hdlc))

write.csv(analytic_df, paste0(path_diabetes_subphenotypes_youth_folder,"/working/cleaned/prov/search only/sepre01_analytic sample.csv"))

complete_cases_df <- source_df %>% 
  dplyr::select(-c("insulinf","glucosef")) %>% 
  dplyr::filter(complete.cases(.)) # N = 335

#write.csv(complete_cases_df, paste0(path_diabetes_subphenotypes_youth_folder,"/working/cleaned/prov/setdy01b_complete cases.csv"))
