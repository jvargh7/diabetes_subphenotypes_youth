...
table_df <- read_csv("prov/analysis/dsy02_descriptive characteristics - total by cluster.csv") %>% 
...
write_csv(table_df,"prov/paper/table_descriptive characteristics by cluster.csv")  

#--------------------------------------------------------------------------------------------------------------------------
## STable 1 - included | excluded

table_df <- read_csv("prov/analysis/dsy02_descriptive characteristics - total by study and cca.csv") %>% 
  dplyr::mutate(selected_rows = case_when(variable %in% mean_vars & est %in% c("mean","sd") ~ 1,
                                          variable %in% median_vars & est %in% c("median","q25","q75") ~ 1,
                                          !variable %in% c(mean_vars,median_vars) ~ 1,
                                          TRUE ~ 0
...
write_csv(table_df,"prov/paper/table_descriptive characteristics by study and cca.csv")  


#-----------------------------------------------------------------------------------------------------------
### STable 2 - study*sex

table_df <- read_csv("prov/analysis/dsy02_descriptive characteristics - total by study and sex.csv") %>% 
  dplyr::mutate(selected_rows = case_when(variable %in% mean_vars & est %in% c("mean","sd") ~ 1,
                                          variable %in% median_vars & est %in% c("median","q25","q75") ~ 1,
                                          !variable %in% c(mean_vars,median_vars) ~ 1,
                                          TRUE ~ 0
...
write_csv(table_df,"prov/paper/table_descriptive characteristics by study and sex.csv")  


#-----------------------------------------------------------------------------------------------------------
### STable 3 - sex*cluster

table_df <- read_csv("prov/analysis/dsy02_descriptive characteristics - total by sex and cluster.csv") %>% 
  dplyr::mutate(selected_rows = case_when(variable %in% mean_vars & est %in% c("mean","sd") ~ 1,
                                          variable %in% median_vars & est %in% c("median","q25","q75") ~ 1,
                                          !variable %in% c(mean_vars,median_vars) ~ 1,
                                          TRUE ~ 0
...
write_csv(table_df,"prov/paper/table_descriptive characteristics by sex and cluster.csv")  


#-----------------------------------------------------------------------------------------------------------
### STable 4 - study

table_df <- read_csv("prov/analysis/dsy02_descriptive characteristics - total by study.csv") %>% 
  dplyr::mutate(selected_rows = case_when(variable %in% mean_vars & est %in% c("mean","sd") ~ 1,
                                          variable %in% median_vars & est %in% c("median","q25","q75") ~ 1,
                                          !variable %in% c(mean_vars,median_vars) ~ 1,
                                          TRUE ~ 0
...
write_csv(table_df,"prov/paper/table_descriptive characteristics by study.csv")  
