...
prep_df <- read_excel("analysis/dsy06_homa2 indices calculation.xlsx") %>% 
  mutate(obs = row_number()) %>% 
...
combined_plot %>% 
  ggsave(plot = ., filename = paste0(path_diabetes_subphenotypes_youth_folder,"/figures/homa2 indices by ratio and cpeptide.jpg"),width = 14, height = 5.5)
